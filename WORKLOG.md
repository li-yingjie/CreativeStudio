...
- Radix Popover 样例:把侧栏底部「设置(外观)」浮层改成 `@radix-ui/react-popover`
  ...
  通过,预览实测:内容完整不截断、在视口内、切主题/关闭正常。(未提交)
...
- 首页输入指令改为「新建项目」逻辑:`submitFromHome` 不再把 prompt 丢进现有
  ...
  首页提交 → 侧栏出现新项目、面包屑切到新项目、右侧空态、需求流启动,无报错。(未提交)
- 重构 Step 2(抽离无状态组件,纯移动):抽出 `Toolbar.tsx`(`FlexAlignGlyph` /
  ...
  toolbar(知识库 跳转/添加)、侧栏树、代码视图均正常,无报错。(未提交)
- 重构 Step 1(瘦身 `VibeCodingPage.tsx`,纯数据外移、零逻辑改动):抽出
  ...
  主文件 10,497 → 10,079 行。tsc 通过,预览实测项目可正常打开、无报错。(未提交)
- 用 TinyPNG 批量压缩 `public/bg`、`public/assets` 下 59 张在用图片(头像≤512px、其余≤1600px,
  ...
  并把「图片必须压缩、保持小体积」写进 CLAUDE.md 规则。(未提交)
- 新增 `WORKLOG.md`(本文件)— 用于持续记录每次改动。(未提交)
- 新增 `CLAUDE.md` / `README.md` — 项目导航(给 AI)与项目介绍(给人)。(未提交)
- 删除 `public/` 下 62 个未引用的孤儿资源(约 51.5M,含 `models/character.glb` 19M、
  `bg/` 一批概念图/视频、`bg/presets|library|voice/` 等);保留动态引用的 `flex-align-*.svg`。(未提交)
