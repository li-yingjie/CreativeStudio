...
- `npm run lint` — ESLint
- `npm test` — Node 回归测试（API 安全、创作者数据、排行榜 Worker）
- `npm run check` — 依次运行 lint、test 和 production build（CI 同步执行）
...
- `tests/` — Node 内置 test runner 的服务端/Worker 回归测试
- `.github/workflows/ci.yml` — Node 22 CI，执行 `npm run check`
