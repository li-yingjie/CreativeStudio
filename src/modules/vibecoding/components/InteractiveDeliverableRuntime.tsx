...
function EditorNode({
  ...
  const editable = Boolean(onSelectionChange)
  const active = editable && selection === id
  ...
  return (
    <div
      data-page-editor-node={id}
      data-page-editor-label={label}
      data-page-editor-selected={active || undefined}
      onPointerDown={handlePointerDown}
      className={`${className} ${editable ? 'transition-[outline-color,box-shadow] hover:outline hover:outline-1 hover:outline-[#3478FF]/55 hover:outline-offset-2' : ''} ${active ? 'z-[25] outline outline-2 outline-[#3478FF] outline-offset-2 shadow-[0_0_0_4px_rgba(52,120,255,0.1)]' : ''}`}
    >
      {children}
    </div>
  )
...
function PhoneStatus({ title, dark = false, editor }: { title: string; dark?: boolean; editor?: DocumentedPageEditorState }) {
  if (editor?.surface.kind !== 'h5' && editor?.surface.titleBar === 'hidden') return null
  const standardTitleBar = editor?.surface.kind !== 'h5' && editor?.surface.titleBar === 'standard'
  return (
    <div className={`relative z-20 flex h-11 items-center justify-between px-4 text-[10px] font-medium ${standardTitleBar ? 'border-b border-black/[0.06] bg-white/95 text-[#161823] backdrop-blur' : dark ? 'text-white' : 'text-[#161823]'}`}>
      <span>9:41</span>
...
function AcgVenueRuntime({ item, editor, selection, onSelectionChange }: RuntimeEditorProps) {
  ...
  return (
    <EditorNode id="page" label="页面实例" selection={selection} onSelectionChange={onSelectionChange} className="relative mx-auto w-[390px] max-w-full overflow-hidden bg-[#FFF4DE] text-[#542B1E] shadow-[0_28px_72px_rgba(31,35,41,0.18)]">
      <RuntimeNotice message={notice} />
      {editorElementVisible(editor, 'hero') ? <EditorNode id="hero" label="主视觉区" selection={selection} onSelectionChange={onSelectionChange} className="relative h-[360px] overflow-hidden bg-[#E95838]">
        <img
        ...
        <PhoneStatus title="抖音 ACG 新春会" dark editor={editor} />
        ...
        <div className="absolute inset-x-5 bottom-5 text-white">
          <EditorNode id="subtitle" label="副标题" selection={selection} onSelectionChange={onSelectionChange} className="w-fit"><p className="text-[10px] font-medium text-white/76">{editor?.subtitle ?? '抖音 ACG 新春会 · 01.09—02.28'}</p></EditorNode>
          <EditorNode id="title" label="主标题" selection={selection} onSelectionChange={onSelectionChange} className="mt-1 w-fit"><h2 className="text-[30px] font-black tracking-[-0.05em]">{editor?.title ?? '开年高燃'}</h2></EditorNode>
          <p className="mt-1 text-[11px] text-white/80">正在浏览 {venue}</p>
          {editor ? <EditorNode id="primaryAction" label="主要按钮" selection={selection} onSelectionChange={onSelectionChange} className="mt-3 w-fit"><button type="button" onClick={() => setNotice(editor.cta)} className="rounded-full bg-white px-3 py-2 text-[9px] font-bold text-[#B43A23]">{editor.cta}</button></EditorNode> : null}
        </div>
      </EditorNode> : null}

      <div className="relative -mt-1 px-4 pb-8">
        {editorElementVisible(editor, 'navigation') ? <EditorNode id="navigation" label="导航与入口" selection={selection} onSelectionChange={onSelectionChange} className="relative"><div className="grid grid-cols-2 gap-2 rounded-2xl bg-white p-1.5 shadow-[0_10px_24px_rgba(128,55,27,0.12)]">
        ...
        {editorElementVisible(editor, 'content') ? <EditorNode id="content" label="核心内容区" selection={selection} onSelectionChange={onSelectionChange} className="relative mt-4 overflow-hidden rounded-2xl bg-white shadow-[0_8px_22px_rgba(128,55,27,0.09)]">
        ...
        {editorElementVisible(editor, 'footer') && (editor?.gameplay.taskEnabled ?? true) ? <EditorNode id="footer" label="任务与回流区" selection={selection} onSelectionChange={onSelectionChange} className="relative mt-4 rounded-2xl bg-[#5A2C22] p-4 text-white">
...
function XiahuaRuntime({ item, editor, selection, onSelectionChange }: RuntimeEditorProps) {
  ...
  if (item.id === 'DLV-XIA-012') {
    ...
    return (
      <EditorNode id="page" label="页面实例" selection={selection} onSelectionChange={onSelectionChange} className="relative mx-auto min-h-[820px] w-[390px] max-w-full overflow-hidden bg-[radial-gradient(circle_at_50%_12%,#FFE990,#FF8156_48%,#632E1E)] text-white shadow-[0_28px_72px_rgba(31,35,41,0.18)]">
        <RuntimeNotice message={notice} /><PhoneStatus title={editor?.title ?? '选择你的夏日搭子'} dark editor={editor} />
        {editorElementVisible(editor, 'hero') ? <EditorNode id="hero" label="主视觉区" selection={selection} onSelectionChange={onSelectionChange} className="relative px-5 pb-10 pt-8 text-center"><p className="text-[10px] text-white/70">进入夜食会场前</p><EditorNode id="title" label="主标题" selection={selection} onSelectionChange={onSelectionChange} className="mt-2"><h2 className="text-[30px] font-black">{editor?.title ?? '选择一匹小马'}</h2></EditorNode><EditorNode id="subtitle" label="副标题" selection={selection} onSelectionChange={onSelectionChange} className="mt-2"><p className="text-[11px] text-white/68">{editor?.subtitle ?? '不同角色会带来专属开场动作'}</p></EditorNode></EditorNode> : null}
        <EditorNode id="content" label="核心内容区" selection={selection} onSelectionChange={onSelectionChange} className="relative flex items-end justify-center gap-1 px-3">
        ...
        <EditorNode id="primaryAction" label="主要按钮" selection={selection} onSelectionChange={onSelectionChange} className="relative mx-auto mt-8 w-[260px]"><button type="button" onClick={() => { setHorseConfirmed(true); setNotice(`已选择${['夜市主理马', '清凉冲浪马', '心仔搭档'][selectedHorse]}，即将进入会场`) }} className="block h-12 w-full rounded-full bg-white text-[13px] font-bold text-[#D94D30] shadow-xl">{horseConfirmed ? '✓ 已选好，进入夜食会场' : editor?.cta ?? '带它出发'}</button></EditorNode>
  ...
  if (item.id === 'DLV-XIA-013') {
    return (
      <EditorNode id="page" label="页面实例" selection={selection} onSelectionChange={onSelectionChange} className="relative mx-auto min-h-[820px] w-[390px] max-w-full overflow-hidden bg-[#1A1010] text-white shadow-[0_28px_72px_rgba(31,35,41,0.18)]">
        <RuntimeNotice message={notice} />
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-black/40 via-transparent to-black/80" /><PhoneStatus title="夜食 AR 会场" dark editor={editor} />
        <EditorNode id="hero" label="主视觉区" selection={selection} onSelectionChange={onSelectionChange} className="absolute inset-x-5 top-20 rounded-2xl border border-white/20 bg-black/30 p-4 backdrop-blur"><EditorNode id="title" label="主标题" selection={selection} onSelectionChange={onSelectionChange} className="w-fit"><p className="text-[9px] text-white/64">{editor?.title ?? 'AR 识别状态'}</p></EditorNode><p className="mt-1 text-[13px] font-semibold">{arStamped ? '打卡已完成：夜市餐桌' : '镜头已识别：夜市餐桌'}</p><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/14"><span className={`block h-full rounded-full bg-emerald-400 transition-all ${arStamped ? 'w-full' : 'w-2/3'}`} /></div></EditorNode>
        <EditorNode id="content" label="核心内容区" selection={selection} onSelectionChange={onSelectionChange} className="absolute inset-x-0 bottom-8 flex flex-col items-center"><EditorNode id="subtitle" label="副标题" selection={selection} onSelectionChange={onSelectionChange} className={`mb-4 rounded-full px-4 py-2 text-[10px] backdrop-blur ${arStamped ? 'bg-emerald-500/90' : 'bg-black/45'}`}>{arStamped ? '已完成 AR 打卡 · 抽卡次数 +2' : editor?.subtitle ?? '移动镜头，把小马放到餐桌上'}</EditorNode><EditorNode id="primaryAction" label="主要按钮" selection={selection} onSelectionChange={onSelectionChange} className="relative"><button type="button" onClick={() => { setArStamped(true); setNotice('AR 夜食打卡成功，获得 2 次抽卡机会') }} className="grid size-16 place-items-center rounded-full border-4 border-white bg-white/30 shadow-xl"><span className="size-11 rounded-full bg-white" /></button><p className="mt-2 text-center text-[10px]">{arStamped ? '重新拍摄' : editor?.cta ?? '打卡拍照'}</p></EditorNode></EditorNode>
      </EditorNode>
    )
  }
  ...
  return (
    <EditorNode id="page" label="页面实例" selection={selection} onSelectionChange={onSelectionChange} className={`relative mx-auto w-[390px] max-w-full overflow-hidden text-[#4B251D] shadow-[0_28px_72px_rgba(31,35,41,0.18)] ${isFood ? 'bg-[#5B2F20]' : 'bg-[#DDF5FF]'}`}>
      <RuntimeNotice message={notice} />
      <PhoneStatus title={editor?.title ?? (isFood ? '夏日夜食指南' : isWaterStageB ? '一起去玩水 · 交换季' : '一起去玩水')} dark={isFood} editor={editor} />
      {editorElementVisible(editor, 'hero') ? <EditorNode id="hero" label="主视觉区" selection={selection} onSelectionChange={onSelectionChange} className={`relative h-[420px] overflow-hidden ${isFood ? 'bg-[#421F18]' : 'bg-gradient-to-b from-[#7BD8FF] to-[#E9FBFF]'}`}>
        {isFood ? <img src="/assets/xiahua/head-kv.png" alt="夏日夜食活动主视觉" className="absolute inset-0 size-full object-cover object-top" /> : <><div className="absolute inset-x-0 top-10 text-center"><EditorNode id="subtitle" label="副标题" selection={selection} onSelectionChange={onSelectionChange}><p className="text-[11px] font-semibold text-[#0E6F98]">{editor?.subtitle ?? '6.30—8.31 · 夏日玩水季'}</p></EditorNode><EditorNode id="title" label="主标题" selection={selection} onSelectionChange={onSelectionChange} className="mt-2"><h2 className="text-[36px] font-black tracking-[-0.08em] text-white [text-shadow:0_3px_0_#159AC8]">{editor?.title ?? '这夏夯爆了'}</h2></EditorNode></div><img src="/assets/xiahua/outfits/beach-vacation.png" alt="夏日玩水角色" className="absolute bottom-0 left-1/2 h-[300px] -translate-x-1/2 object-contain" /></>}
        {isFood && editor ? <div className="absolute inset-x-5 bottom-16 text-center text-white"><EditorNode id="title" label="主标题" selection={selection} onSelectionChange={onSelectionChange}><h2 className="text-[28px] font-black drop-shadow">{editor.title}</h2></EditorNode><EditorNode id="subtitle" label="副标题" selection={selection} onSelectionChange={onSelectionChange} className="mt-1"><p className="text-[10px] text-white/76">{editor.subtitle}</p></EditorNode></div> : null}
        {editorElementVisible(editor, 'navigation') ? <EditorNode id="navigation" label="导航与入口" selection={selection} onSelectionChange={onSelectionChange} className="absolute inset-x-0 bottom-4 flex justify-center gap-2"><button type="button" onClick={() => setInfoPanel('rules')} className="rounded-full bg-black/35 px-4 py-2 text-[9px] text-white backdrop-blur">规则</button>{editor?.gameplay.shareEnabled ?? true ? <button type="button" onClick={() => setInfoPanel('share')} className="rounded-full bg-black/35 px-4 py-2 text-[9px] text-white backdrop-blur">分享</button> : null}</EditorNode> : null}
      </EditorNode> : null}

      <div className={`relative px-4 pb-9 ${isFood ? 'bg-[#5B2F20]' : 'bg-[#DDF5FF]'}`}>
        {editorElementVisible(editor, 'content') ? <EditorNode id="content" label="核心内容区" selection={selection} onSelectionChange={onSelectionChange} className={`relative -mt-1 rounded-2xl p-4 shadow-xl ${isFood ? 'bg-[#F7C89D]' : 'bg-white'}`}>
          ...
          <EditorNode id="primaryAction" label="主要按钮" selection={selection} onSelectionChange={onSelectionChange} className="mt-4"><button type="button" onClick={drawCard} className="flex h-12 w-full items-center justify-center rounded-full bg-[#FF3F2D] text-[14px] font-black text-white shadow-[0_8px_18px_rgba(255,63,45,0.28)]"><Gift className="mr-2 size-4" />{editor?.cta ?? (isFood ? `抽夏日夜食 · ${draws} 次` : `抽夏日装备 · ${draws} 次`)}</button></EditorNode>
...
function SpringGalaRuntime({ item, editor, selection, onSelectionChange }: RuntimeEditorProps) {
  ...
  return (
    <EditorNode id="page" label="页面实例" selection={selection} onSelectionChange={onSelectionChange} className="relative mx-auto w-[390px] max-w-full overflow-hidden bg-[#FFF0DF] text-[#6D201C] shadow-[0_28px_72px_rgba(31,35,41,0.18)]">
      <RuntimeNotice message={notice} />
      {editorElementVisible(editor, 'hero') ? <EditorNode id="hero" label="主视觉区" selection={selection} onSelectionChange={onSelectionChange} className="relative h-[405px] overflow-hidden bg-[#F33C35]"><img src="/assets/figma-deliverables/spring-gala/activity-header.png" alt="春晚活动主视觉" className="absolute inset-0 size-full object-cover" /><div className="absolute inset-0 bg-gradient-to-b from-black/10 via-transparent to-[#BD2A24]/72" /><PhoneStatus title={editor?.title ?? '上抖音 看春晚'} dark editor={editor} /><div className="absolute inset-x-5 bottom-6 text-white"><EditorNode id="subtitle" label="副标题" selection={selection} onSelectionChange={onSelectionChange} className="w-fit"><p className="text-[10px] text-white/74">{editor?.subtitle ?? '2026 年 2 月 16 日 20:00 直播'}</p></EditorNode><EditorNode id="title" label="主标题" selection={selection} onSelectionChange={onSelectionChange} className="mt-1 w-fit"><h2 className="text-[32px] font-black tracking-[-0.05em]">{editor?.title ?? '上抖音 看春晚'}</h2></EditorNode>{editor ? <EditorNode id="primaryAction" label="主要按钮" selection={selection} onSelectionChange={onSelectionChange} className="mt-3 w-fit"><button type="button" onClick={() => setNotice(editor.cta)} className="rounded-full bg-white px-3 py-2 text-[9px] font-bold text-[#B62925]">{editor.cta}</button></EditorNode> : null}</div></EditorNode> : null}
      <div className="px-4 pb-9">
        {editorElementVisible(editor, 'navigation') ? <EditorNode id="navigation" label="导航与入口" selection={selection} onSelectionChange={onSelectionChange} className="relative -mt-3 overflow-hidden rounded-2xl bg-[#741514] p-3 text-white shadow-xl"><div className="flex items-center justify-between pb-3"><div><span className="rounded bg-[#FF3C44] px-2 py-1 text-[8px]">演示状态 · 直播中</span><p className="mt-2 text-[12px] font-bold">春节联欢晚会节目单</p></div><button type="button" onClick={() => { setFollowed((value) => !value); setNotice(followed ? '已取消关注' : '已关注官方春晚直播') }} className={`rounded-full border px-3 py-2 text-[8px] ${followed ? 'border-white bg-white text-[#7B1A18]' : 'border-white/24'}`}>{followed ? '✓ 已关注' : '＋ 关注'}</button></div><button type="button" onClick={() => setIsPlaying((value) => !value)} aria-label={isPlaying ? '暂停直播' : '播放直播'} className="group relative block aspect-video w-full overflow-hidden rounded-xl"><img src={channelCover} alt={`${channel}直播画面`} className="size-full object-cover" /><span className="absolute inset-0 bg-black/18" /><span className="absolute left-1/2 top-1/2 grid size-12 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full bg-white/90 text-[#D52F2A] shadow-lg">{isPlaying ? <Pause className="size-4" /> : <Play className="ml-0.5 size-4" />}</span>{isPlaying ? <span className="absolute bottom-3 left-3 flex items-center gap-1.5 rounded-full bg-black/55 px-2.5 py-1.5 text-[8px]"><i className="size-1.5 animate-pulse rounded-full bg-red-500" />正在播放 · {channel}</span> : null}</button><div className="mt-3 grid grid-cols-3 gap-1 rounded-xl bg-black/15 p-1">{(['主机位', '竖屏', '手语'] as const).map((label) => <button key={label} type="button" onClick={() => { setChannel(label); setNotice(`已切换到${label}频道`) }} className={`rounded-lg py-2 text-[8px] ${channel === label ? 'bg-white text-[#7B1A18]' : 'text-white/72'}`}>{label}</button>)}</div></EditorNode> : null}

        {editorElementVisible(editor, 'content') ? <EditorNode id="content" label="核心内容区" selection={selection} onSelectionChange={onSelectionChange} className="relative"><section className="mt-4 rounded-2xl bg-white p-4"><div className="flex items-center justify-between"><div><p className="text-[9px] text-[#B17B6E]">{editor?.gameplay.packageName ?? '同一节目数据源'}</p><h3 className="mt-1 text-[17px] font-black">今晚节目单</h3></div><button type="button" onClick={() => setProgramExpanded((value) => !value)} className="text-[8px] font-semibold text-[#D63A31]">{programExpanded ? '收起' : '查看完整节目单'}</button></div><div className="mt-3 divide-y divide-[#F2E4DD]">{[['20:00', '开场秀 · 万马奔腾'], ['20:18', '相声 · 马上有福'], ['20:42', '歌曲 · 春风到家'], ['21:10', '特别节目 · 温暖上线']].slice(0, programExpanded || full ? 4 : 2).map(([time, title], index) => <button key={time} type="button" onClick={() => setNotice(`已预约：${title}`)} className="flex w-full items-center gap-3 py-3 text-left"><span className="w-9 text-[9px] font-semibold text-[#D63A31]">{time}</span><span className="min-w-0 flex-1 truncate text-[10px] font-semibold">{title}</span><span className="text-[8px] text-[#6D201C]/34">{index === 0 && isPlaying ? '直播中' : '预约'}</span></button>)}</div></section>{editor?.gameplay.taskEnabled ?? true ? <section className="mt-4 rounded-2xl bg-white p-4"><div className="flex items-center justify-between"><div><p className="text-[9px] text-[#B17B6E]">互动进行中</p><h3 className="mt-1 text-[18px] font-black">骏马迎新春 开运福满抽</h3></div><span className="rounded-full bg-[#FFF0E9] px-2 py-1 text-[8px] text-[#D63A31]">剩余 {remainingChances} 次</span></div><div className="mt-4 flex justify-between gap-2">{['豆包 AI 耳机', '火山抱枕', '限定键盘', '新春红包'].map((label, index) => <div key={label} className="min-w-0 flex-1 text-center"><div className={`mx-auto grid aspect-square w-full place-items-center rounded-xl ${index === 3 ? 'bg-[#FFE5AA]' : 'bg-[#FFF4EE]'}`}><Gift className="size-5 text-[#F04D43]" /></div><p className="mt-1 truncate text-[7px]">{label}</p></div>)}</div><EditorNode id="primaryAction" label="主要按钮" selection={selection} onSelectionChange={onSelectionChange} className="mt-4"><button type="button" disabled={remainingChances === 0} onClick={() => setLotteryOpen(true)} className="h-12 w-full rounded-full bg-gradient-to-r from-[#FF3D66] to-[#FF5B48] text-[13px] font-black text-white shadow-[0_8px_18px_rgba(255,61,102,0.22)] disabled:cursor-not-allowed disabled:opacity-48">{editor?.cta ?? (remainingChances ? '开始抽奖' : '本次机会已使用')}</button></EditorNode><p className="mt-2 text-center text-[8px] text-[#6D201C]/36">每日参与上限 {editor?.gameplay.dailyLimit ?? 1} 次；正式发布前必须绑定库存与履约</p></section> : null}</EditorNode> : null}

        {editorElementVisible(editor, 'footer') ? <EditorNode id="footer" label="任务与回流区" selection={selection} onSelectionChange={onSelectionChange} className="relative mt-4 rounded-2xl bg-white p-4"><div className="flex items-center justify-between"><h3 className="text-[17px] font-black">往年春晚</h3><span className="text-[8px] text-[#6D201C]/38">结束态回放</span></div><div className="mt-3 grid grid-cols-4 gap-2">{['2024', '2023', '2022', '2021'].map((year) => <button key={year} type="button" onClick={() => { setSelectedYear(year); setNotice(`正在打开 ${year} 春晚回放`) }} className={`overflow-hidden rounded-lg text-white ${selectedYear === year ? 'bg-[#FF3C44] ring-2 ring-[#FFB29E]' : 'bg-[#AA1522]'}`}><div className="grid aspect-[3/4] place-items-center text-[18px] font-black">春</div><p className="bg-white py-1.5 text-[8px] text-[#6D201C]">{year} 春晚</p></button>)}</div></EditorNode> : null}
...
function EvernightRuntime({ item, editor, selection, onSelectionChange }: RuntimeEditorProps) {
  ...
  return (
    <EditorNode id="page" label="页面实例" selection={selection} onSelectionChange={onSelectionChange} className="relative mx-auto w-[390px] max-w-full overflow-hidden bg-[#21115C] text-white shadow-[0_28px_72px_rgba(31,35,41,0.2)]">
      <RuntimeNotice message={notice} />
      {editorElementVisible(editor, 'hero') ? <EditorNode id="hero" label="主视觉区" selection={selection} onSelectionChange={onSelectionChange} className={`relative overflow-hidden bg-[#301879] ${taskOnly ? 'h-[300px]' : 'h-[480px]'}`}><img src="/assets/figma-deliverables/evernight/campaign-cover.png" alt="永夜星河抽卡活动主视觉" className="absolute inset-0 size-full object-cover" /><div className="absolute inset-0 bg-gradient-to-b from-black/18 via-transparent to-[#2A146D]" /><PhoneStatus title={editor?.title ?? (taskOnly ? '永夜星河 · 任务中心' : '永夜星河 · 独星河小卡')} dark editor={editor} /><div className="absolute inset-x-6 bottom-8 text-center"><EditorNode id="subtitle" label="副标题" selection={selection} onSelectionChange={onSelectionChange}><p className="text-[10px] text-[#E5D8FF]">{editor?.subtitle ?? '《永夜星河》抖音抽卡活动'}</p></EditorNode><EditorNode id="title" label="主标题" selection={selection} onSelectionChange={onSelectionChange} className="mt-2"><h2 className={`${taskOnly ? 'text-[25px]' : 'text-[30px]'} font-black tracking-[-0.04em] text-white [text-shadow:0_0_20px_#A76CFF]`}>{editor?.title ?? (taskOnly ? '完成任务，领取抽卡次数' : <>独星河小卡<br />开启快乐征途</>)}</h2></EditorNode></div></EditorNode> : null}
      <div className="relative -mt-4 px-4 pb-10">
        {editorElementVisible(editor, 'navigation') ? <EditorNode id="navigation" label="导航与入口" selection={selection} onSelectionChange={onSelectionChange} className={`relative grid rounded-full bg-[#2A1269] p-1 shadow-lg ${taskOnly ? 'grid-cols-2' : 'grid-cols-3'}`}>{(taskOnly ? ['任务', '图鉴'] as const : ['抽卡', '任务', '图鉴'] as const).map((label) => <button key={label} type="button" onClick={() => setTab(label)} className={`h-10 rounded-full text-[11px] font-bold ${tab === label ? 'bg-[#8F5CFF] text-white' : 'text-white/60'}`}>{label}</button>)}</EditorNode> : null}

        {editorElementVisible(editor, 'content') && tab === '抽卡' && !taskOnly ? <EditorNode id="content" label="核心内容区" selection={selection} onSelectionChange={onSelectionChange} className="relative mt-4 rounded-2xl border border-white/10 bg-white/8 p-4 text-center backdrop-blur"><div className="relative mx-auto h-[260px]"><img src="/assets/evernight/card-collection.webp" alt="独星河卡池" className="size-full object-contain" /><button type="button" onClick={() => setNotice('已切换到下一张卡池主卡')} aria-label="下一张卡" className="absolute right-0 top-1/2 grid size-10 -translate-y-1/2 place-items-center rounded-full bg-black/30"><ChevronRight className="size-5" /></button></div><p className="mt-2 text-[11px] text-[#E8DEFF]">{editor?.gameplay.packageName ?? '抖音独占卡 DYR'} · 已收集 {collected}/7</p><div className="mt-4 grid grid-cols-7 gap-1">{Array.from({ length: 7 }).map((_, index) => <button key={index} type="button" onClick={() => setNotice(index < collected ? `第 ${index + 1} 张卡已收集` : '这张卡尚未解锁')} className={`aspect-[3/4] rounded-md border ${index < collected ? 'border-[#74E2FF] bg-gradient-to-b from-[#7C5EFF] to-[#25144F]' : 'border-white/8 bg-black/38'}`}>{index < collected ? <Sparkles className="mx-auto size-3 text-[#8DEBFF]" /> : <span className="text-[9px] text-white/30">🔒</span>}</button>)}</div><EditorNode id="primaryAction" label="主要按钮" selection={selection} onSelectionChange={onSelectionChange} className="mt-5"><div className="grid grid-cols-2 gap-3"><button type="button" onClick={() => draw(1)} className="h-12 rounded-full bg-gradient-to-r from-[#8C62FF] to-[#B05CFF] text-[13px] font-black">{editor?.cta ?? '单次抽卡'}</button><button type="button" onClick={() => draw(10)} className="h-12 rounded-full bg-gradient-to-r from-[#FFD85A] to-[#FFB743] text-[13px] font-black text-[#4B2C00]">十次连抽</button></div></EditorNode><p className="mt-3 text-[10px] text-white/62">你有 <strong className="text-[#FFD45C]">{chances}</strong> 次抽卡机会</p><p className="mt-2 text-[8px] text-white/38">概率、保底与库存为待接入业务数据，不在 Demo 中伪造</p></EditorNode> : null}

        {editorElementVisible(editor, 'content') && tab === '图鉴' ? <EditorNode id="content" label="核心内容区" selection={selection} onSelectionChange={onSelectionChange} className="relative mt-4 rounded-2xl border border-white/10 bg-white/8 p-4"><div className="flex items-center justify-between"><div><p className="text-[9px] text-[#C8B5F8]">COLLECTION</p><h3 className="mt-1 text-[18px] font-black">独星河图鉴 {collected}/7</h3></div><span className="rounded-full bg-[#8F5CFF] px-3 py-1.5 text-[9px]">{Math.round(collected / 7 * 100)}%</span></div><div className="mt-4 grid grid-cols-2 gap-3">{['DYR 独占', 'SSR 星河', 'SP 联名', 'SR 角色', 'R 剧照', '限定签名'].map((label, index) => <button key={label} type="button" onClick={() => setNotice(index < collected ? `${label}卡 · 已收藏` : `${label}卡 · 可在卡池获得`)} className={`relative aspect-[3/4] overflow-hidden rounded-xl border p-3 text-left ${index < collected ? 'border-[#77DEFF] bg-gradient-to-br from-[#7D5EFF] via-[#4C2A9C] to-[#1F124E]' : 'border-white/8 bg-black/28 grayscale'}`}><img src={`/assets/figma-deliverables/evernight/card-frame-${index === 0 ? 'dyr' : index === 1 ? 'ssr' : index === 2 ? 'sp' : index === 3 ? 'sr' : 'r'}.png`} alt="" className="absolute inset-0 size-full object-cover opacity-55" /><span className="relative text-[9px] font-bold">{label}</span><span className="absolute bottom-3 left-3 text-[8px] text-white/58">{index < collected ? '已获得' : '未解锁'}</span></button>)}</div>{editor?.gameplay.shareEnabled ?? true ? <button type="button" disabled={shared || collected === 0} onClick={() => { setShared(true); setNotice('分享卡已生成，可带已获得卡片回流') }} className="mt-4 h-11 w-full rounded-full border border-[#A98BFF] bg-white/8 text-[10px] font-bold disabled:opacity-42">{shared ? '✓ 分享卡已生成' : '用已获得卡片生成分享卡'}</button> : null}</EditorNode> : null}

        {editorElementVisible(editor, 'footer') && (editor?.gameplay.taskEnabled ?? true) && tab === '任务' ? <EditorNode id="footer" label="任务与回流区" selection={selection} onSelectionChange={onSelectionChange} className="relative mt-4 rounded-2xl bg-[#49318A] p-4"><div className="flex items-end justify-between"><div><p className="text-[9px] text-[#C8B5F8]">TASKS · 每日上限 {editor?.gameplay.dailyLimit ?? 3} 次</p><h3 className="mt-1 text-[18px] font-black">做任务得抽卡机会</h3></div><span className="text-[9px] text-white/48">{tasks.filter(Boolean).length}/{tasks.length} 已完成</span></div><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/10"><span className="block h-full rounded-full bg-[#F177D7] transition-all" style={{ width: `${tasks.filter(Boolean).length / tasks.length * 100}%` }} /></div><div className="mt-3 space-y-2">{taskLabels.map((label, index) => <div key={label} className="flex items-center gap-3 rounded-xl bg-white/8 px-3 py-3"><span className={`grid size-8 place-items-center rounded-xl ${tasks[index] ? 'bg-[#F177D7]' : 'bg-white/10'}`}>{tasks[index] ? '✓' : index + 1}</span><div className="min-w-0 flex-1"><p className="truncate text-[10px] font-semibold">{label}</p><p className="mt-0.5 text-[8px] text-white/46">完成后抽卡机会 +{index === 0 ? 3 : 10}</p></div><button type="button" disabled={tasks[index]} onClick={() => { setTasks((current) => current.map((value, taskIndex) => taskIndex === index ? true : value)); setChances((value) => value + (index === 0 ? 3 : 10)); setNotice(`${label}完成，抽卡机会已到账`) }} className={`rounded-full px-3 py-2 text-[9px] ${tasks[index] ? 'bg-white/8 text-white/30' : 'bg-[#925CFF] text-white'}`}>{tasks[index] ? '已完成' : '去完成'}</button></div>)}</div>{!taskOnly ? <button type="button" onClick={() => setTab('抽卡')} className="mt-4 h-11 w-full rounded-full bg-white text-[10px] font-bold text-[#5C36A8]">带着 {chances} 次机会去抽卡</button> : <button type="button" onClick={() => setTab('图鉴')} className="mt-4 h-11 w-full rounded-full bg-white text-[10px] font-bold text-[#5C36A8]">查看图鉴进度 {collected}/7</button>}</EditorNode> : null}
      </div>
...
export default function InteractiveDeliverableRuntime({
  ...
  const runtimeProps = { item, editor, selection, onSelectionChange }
  if (activityCase.code === 'CASE-ACG-CNY-2026') return <AcgVenueRuntime {...runtimeProps} />
  if (activityCase.code === 'CASE-UGC-SUMMER-2026') return <XiahuaRuntime {...runtimeProps} />
  if (activityCase.code === 'CASE-GALA-2026') return <SpringGalaRuntime {...runtimeProps} />
  return <EvernightRuntime {...runtimeProps} />
}
